...
void qemu_dxgi_swapchain_ResizeBuffers(struct qemu_syscall *call)
{
    ...
    /* if bufferCount == 0, then the existing buffer count is preserved, so we need to see what that is */
    if (c->buffer_count == 0) {
        DXGI_SWAP_CHAIN_DESC desc;
        ...
        c->buffer_count = desc.BufferCount;
    }

    host_buffers = HeapAlloc(GetProcessHeap(), 0, c->buffer_count * sizeof(*host_buffers));
    if (!host_buffers)
        WINE_WARN("Out of memory\n");

    for (i = 0; i < c->buffer_count; ++i)
    {
        c->super.iret = IDXGISwapChain1_GetBuffer(swapchain->host, i, &IID_IDXGISurface1, (void **)&buffer);
        if (FAILED(c->super.iret)) {
